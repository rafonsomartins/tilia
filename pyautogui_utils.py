...
def force_clipboard_content(rel_start_x, rel_start_y, rel_end_x, rel_end_y, sap_paste=False, absolute=False, direction='Down', horizontal=False):
	down = 1
	if direction == 'up':
		down = -1
	...
	if not message:
		if not horizontal:
			return force_clipboard_content(rel_start_x, rel_start_y + (0.008 * down), rel_end_x, rel_end_y + (0.008 * down), sap_paste, absolute)
		else:
			return force_clipboard_content(rel_start_x + (0.008 * down), rel_start_y, rel_end_x + (0.008 * down), rel_end_y, sap_paste, absolute)
	else:
		return message
